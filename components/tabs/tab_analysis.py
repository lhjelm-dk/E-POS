...
def _render_geo_diagnostics(ctx) -> None:
    ...
    st.subheader("Risk Summary: Incompleteness, Uncertainty Index & Key Drivers")

    # ── Incompleteness — the native ESL uncertainty (headline white band) ──
    ...
    if _esl_conflict:
        ...
        _eu_head = (f"Incompleteness: ⚠️ overcommitted by "
        ...
        _eu_head = f"Incompleteness: {_esl_white * 100:.1f}% unknown (white band)"
        _eu_body = (f"{_esl_white * 100:.1f}% of the headline rests on the <b>unknown</b> band, not "
                    ...
                    "its width <i>is</i> the Incompleteness. It is fixed by the evidence and does not "
                    "change with the stance w.")
    st.markdown(
    ...
    st.caption(
        "**Incompleteness and the Uncertainty Index are two different things.** "
        "**Incompleteness** (above) is the white band: how much of the headline has no evidence "
        "either way. It is fixed by the evidence and does not depend on the stance w. "
        "The **Uncertainty Index** (below) is a *point-estimate* adequacy check — whether your two "
        "weakest pillar chances are jointly strong enough — and it *does* move with the stance w. "
        "It is the same metric shown on the Classic POS page."
    )
    ...
    render_pg_ui_trajectory(
        ...
        reading_guide=False,   # one combined expander rendered below instead
    )

    with st.expander(
        "📐 Reading this plot — the ESL envelope & whether UI(ESL) predicts success "
        "(experimental)", expanded=False,
    ):
        st.markdown(
            "##### Reading the plot\n"
            "- **Blue curve:** the stance trajectory, sweeping w from 0 (pessimistic) to 1 "
            "(optimistic); each point is `(P(G, ESL), UI)` at that stance. The **★** is the current "
            "stance; the **Bel / Pl** pills above are the w = 0 and w = 1 ends.\n"
            "- **Background:** red (UI < 0) → green (UI > 0). If the trajectory crosses `UI = 0`, the "
            "robust-vs-risk-driven reading depends on your stance.\n"
            "- **Grey curves:** the envelope (next).\n\n"
            "##### The two bounds\n"
            "**Lower `UI = 2·x − 1` (exact, hard).** Worst case: two pillars committed-positive "
            "(Pg = 1) and two carrying all the uncertainty. At stance *w* those two sit at Pg = *w*, "
            "so P(G, ESL) = *w* and UI = 2*w* − 1; eliminating *w* gives the straight diagonal. The "
            "trajectory can **never** drop below it (checked over millions of configs).\n\n"
            "**Upper `UI = 2·x^(1/4) − 1` (balanced reference, not a cap).** Best-balanced case: all "
            "four pillars equal, no white. It caps an ordinary prospect, but a single near-veto "
            "pillar (Pg ≈ 0) crashes the headline toward 0 while the others keep UI moderate, so the "
            "point can rise above it at very low P(G). (A reminder that the two-weakest UI "
            "under-reflects a single decisive veto.)\n\n"
            "##### Why the ESL floor differs from the classical (Rose) UI floor\n"
            "Same worst case (two pillars solid, two unknown) at the neutral stance w = 0.5:\n\n"
            "| | combine rule | headline | UI |\n"
            "|---|---|---|---|\n"
            "| **Classic** | multiply `w · w = w²` | **25%** | 0% |\n"
            "| **ESL** | combine masses, Policy P once `w` | **50%** | 0% |\n\n"
            "Same prospect, same UI, but 25% on the Classic axis vs 50% on the ESL axis. Classic "
            "**squares** the headline (`x = w²` → `UI = 2·√x − 1`, a curve); ESL keeps it linear "
            "(`x = w` → `UI = 2·x − 1`, the diagonal). That horizontal shift is the "
            "**ESL − Classic gap**."
        )
        st.markdown(
            "##### Does a positive UI(ESL) mean success? (experimental, uncalibrated)\n"
            "For the **classical** UI there is an industry regularity: drilled successes tend to "
            "cluster at **UI > 0**. Whether the same holds for **UI(ESL)** is an open, untested "
            "question, and the honest answer is *not automatically* — for a structural reason worth "
            "understanding.\n\n"
            "**The classical UI quietly mixed two things.** In classical risking an uncertain pillar "
            "gets a *lower* chance (conservatism), so a low classical chance means both "
            "*unfavourable* **and** *uncertain*. The classical UI catches both, and both kinds of "
            "prospect drill dry more often — which is part of why UI > 0 discriminated.\n\n"
            "**ESL un-mixes them on purpose.** Unfavourable evidence (red) lowers UI(ESL); "
            "uncertainty (white) does **not** — at neutral stance it sits near 0.5 and is exported "
            "to the **Incompleteness** index instead. So ESL splits the classical UI's signal into "
            "two channels: **UI(ESL) = the unfavourability part**, **Incompleteness = the ignorance "
            "part**.\n\n"
            "**Consequences (hypotheses to test against drilled outcomes):**\n"
            "- UI(ESL) > 0 *alone* is probably a **weaker** success signal than classical UI > 0, "
            "because the uncertainty penalty has been moved out. You likely want **UI(ESL) > 0 *and* "
            "low Incompleteness**.\n"
            "- The closest analogue to classical UI > 0 is **UI(ESL) at w = 0** (the Bel end of the "
            "trajectory) **> 0**: evaluating at the pessimistic stance re-folds the uncertainty back "
            "in, reconstructing the classical conservatism.\n"
            "- UI(ESL) at the neutral stance will tend to **over-predict** success; the gap between "
            "the neutral and the w = 0 reading is essentially the prospect's white band.\n\n"
            "So the *structure* says where to look (the Bel-stance UI, or the UI–Incompleteness "
            "pair), but the *threshold* is genuinely \"time will tell\": it needs a drilled-outcome "
            "calibration, and it will **not** be exactly 0 for the neutral-stance ESL UI."
        )
