...
> P(G, ESL), Incompleteness, Uncertainty Index, CCM scatter, and the P(G, Classic) derived view
...
| **I** — incompleteness ("don't know") | the **white** of the flag | `White`, the stance `w`, the Bel/Pl envelope, the **Incompleteness** index (= Pl − Bel = white) |
| **R** — randomness | classical probability | the Bayes DFI likelihoods (Gaussian densities), volumetrics |

> **Incompleteness vs Uncertainty Index — two different things.**
> **Incompleteness** measures the white band: `Pl − Bel = 1 − S_for − S_against`. It is fixed by the evidence and does **not** depend on the stance `w`.
> The **Uncertainty Index** (after MacKay 1995) is a *point-estimate adequacy* check: `MIN(P_pillar) + 2nd-MIN(P_pillar) − 1`, i.e. whether the two weakest pillar chances are jointly strong enough. It **does** move with `w`, and it is the metric on both the ESL and the Classic POS trajectory plots.
