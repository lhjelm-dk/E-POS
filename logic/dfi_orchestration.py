...
def geox_pdfi_value(dhi_index: float, calib, class_name: str, sd_mode: str) -> float:
    """P(DFI|case) for one SAAM class, scaled to match the SLB GeoX input convention."""
...
def _build_decision_narrative(prior_e, post_e, prior_c, post_c,
    ...
    return (
        ...
        f"R_SAAM = **{r:.2f}**, DHI Volume Weight = **{v:.2f}**. "
        f"{method_agreement}"
    )
...
def build_simm_decision_narrative(prior_e, post_e, prior_c, post_c, r,
    ...
    SAAM's R_SAAM/Volume-Weight/DHI-Index it reports a single likelihood ratio
...
def _build_summary_text(prospect_title, analyst, basin, review_date, w_cur,
    ...
    lines.append(f"  HC fluid type (SAAM class): {CLASS_DISPLAY.get(fluid_type, fluid_type)}")
    ...
    lines.append(f"  R_SAAM (ESL)         : {post_e.r_saam:6.2f}")
    lines.append(f"  DHI Volume Weight (ESL)      : {post_e.dhi_volume_weight:6.2f}")
    lines.append(f"  R_SAAM (Classic)     : {post_c.r_saam:6.2f}")
    ...
    lines.append("Methodology: Bayesian DFI update with Gaussian likelihoods from SAAM calibration.")
    return "\n".join(lines)
